...
3. Architecture analysis commands (`module:graph`, `module:impact`, `module:why`, `module:list`) plus scaffolding/lifecycle commands (`true-modular:setup`, `module:make`, `module:make:migration`, `module:seed`) and the module registry cache (`true-modular:cache` / `true-modular:clear`, hooked into `optimize` / `optimize:clear`).
...
`Generators/ModuleGenerator` scaffolds a new module from **stubs, not heredocs** — `stubs/module/` in the package, overridable by `vendor:publish --tag=true-modular-stubs` into the app's `stubs/true-modular/module/`, or by rebinding `ModuleGenerator` (the command resolves it from the container). The stub SET is the file set: one `*.stub` produces one file at the same relative path, so adding/removing a stub adds/removes a file. Placeholders (`{{ slug }}` and `{{slug}}` both work) are substituted in contents **and paths**; `{{ namespaceEscaped }}` exists because JSON has no single-backslash escape. When touching this, keep the packaged stubs' rendered output byte-identical — `tests/Unit/ModuleGeneratorTest.php` pins it, and consumers diff scaffolds.
